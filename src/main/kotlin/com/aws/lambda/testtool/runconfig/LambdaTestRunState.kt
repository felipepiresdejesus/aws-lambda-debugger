...
class LambdaTestRunState(
    ...
    private fun buildLambdaCommandLine(
        ...
        // On Unix, we can use exec -a to set the process name
        // On Windows, the process will show as the DLL name, but we set env vars to help identify it
        val isWindows = System.getProperty("os.name").lowercase().contains("windows")
        
        val commandLine = if (isWindows) {
            // Windows: Just use dotnet directly (process name will be the DLL)
            GeneralCommandLine("dotnet", dllPath)
                .withWorkDirectory(workingDir)
        } else {
            // Unix: Use exec -a to set a descriptive process name
            // IMPORTANT: Use the project name (sanitized) so the IDE displays it correctly
            // Must be different from "LambdaTestTool" to avoid filtering conflicts
            // Keep dots but remove other special chars and spaces from project name for process name
            ...
            val processName = if (sanitizedProjectName.isNotEmpty() && sanitizedProjectName != "LambdaTestTool") {
                ...
                "LambdaFunction" // Fallback if project name is invalid or conflicts
            }
            val shellCommand = "exec -a '$processName' dotnet '$dllPath'"
            LOG.info("Setting Lambda function process name to: $processName (project: ${lambdaProject.name}, different from Test Tool to avoid filtering conflicts)")
